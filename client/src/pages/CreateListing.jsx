    ...
    const handleImageSubmit=(e)=>{
        if(files.length>0 && files.length+formData.imageUrls.length<=6){
            ...
            Promise.all(promises).then((urls)=>{
                ...
                setUploading(false);
                setImageUploadError("Image upload failed. Please try again.(2MB per image)");
            })
            
        }else{
            setUploading(false);
            setImageUploadError("You can only upload up to 6 images, including the cover image.");
        }
    };
